...
Return JSON only:
{
  ...
  synthesizeLocalMessage(profile, strategy, phase, reward = "1.50") {
    ...
    if (phase === "before") {
      if (strategy === "relatedness") {
        return `The concrete goal of this task is described as "${objective}". We value the time you're taking to do this. Whatever you submit will be carefully referenced as we organize and review the overall material. ${fatiguePhrase} ${taskLengthPhrase}. The effort you put into reviewing each item will also be recorded as part of this meaningful process.`;
      }
      if (strategy === "competence") {
        return `For this task, a careful eye matters more than clicking quickly. As you work toward the stated goal of "${objective}", human contextual judgment is what determines data quality. And the results you submit are used to check the overall quality and consistency of the data. Since ${taskLengthPhrase}, please proceed at a comfortable pace within the given criteria. For ambiguous items, just review the guidelines again and choose whichever option seems most appropriate.`;
      }
      if (strategy === "autonomy") {
        return `A quick note before you begin. As you work toward the goal of "${objective}", you may run into ambiguous items — when that happens, it's fine not to force a guess. ${autonomyPhrase}. ${fatiguePhrase} feel free to work at whatever pace is comfortable for you, and take a short break if you need one. The judgments gathered this way are used to check the quality of the overall results.`;
      }
  ...
  generateInterventions(title, category, description, riskLevel, fatigueLevel, objective, socialImpact, workerContext, reward = "1.50", singleTaskLimitMinutes = 15, taskType = "") {
    ...
    return {
      beforeOptions,
      afterOptions,
      beforeLabels: ["Connection & Contribution", "Confidence in Judgment", "Autonomy & Choice"],
